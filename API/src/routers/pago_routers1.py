...
from src.config.utils import get_api_key  # Importa la función de validación de API Key

pago1_router = APIRouter()

# Crear un pago
@pago1_router.post('/pago', status_code=HTTP_201_CREATED, tags=['Pago_Vecino'], dependencies=[Depends(get_api_key)])
...
@pago1_router.get('/pago', tags=['Pago_Vecino'], response_model=List[pago_vecino], dependencies=[Depends(get_api_key)])
def mostrar_pagos():
    with engine.connect() as conn:
...
@pago1_router.get('/pago/{vecino_dni}', response_model=List[pago_vecino], tags=['Pago_Vecino'], dependencies=[Depends(get_api_key)])
...
@pago1_router.put('/pago/{id_pago}', tags=['Pago_Vecino'], response_model=pago_update, dependencies=[Depends(get_api_key)])
...
# Eliminar un pago
@pago1_router.delete('/pago/{id_pago}', tags=['Pago_Vecino'], status_code=HTTP_204_NO_CONTENT, dependencies=[Depends(get_api_key)])
